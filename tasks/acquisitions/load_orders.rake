...
namespace :acquisitions do
  ...
  desc 'transform SUL orders from YAML to JSON'
  task :transform_orders_sul do
  ...
  task :load_orders_sul do
    ...
    Dir.each_child(order_json_dir) do |file|
      File.open("#{order_json_dir}/#{file}", 'w') do |f|
        f.puts orders_post(folio_composite_orders).to_json
      end
    end
  end

  desc 'transform LAW orders from YAML to JSON'
  task :transform_orders_law do
