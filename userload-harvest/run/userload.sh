...
do
    let batch=batch+1
    printf '%s\n' "${array[@]}" > $OUT/tmp.xml
    STAGE="${STAGE}" ruby bin/userload.rb $OUT/tmp.xml $batch >> $LOG/userload.log 2>> $LOG/userload-err.log
    rm $OUT/tmp.xml
done < $HARVEST

cat $LOG/userload-err.log | mailx -s 'Folio Userload Errors' sul-unicorn-devs@lists.stanford.edu
