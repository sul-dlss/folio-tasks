...
describe 'load SUL orders rake tasks' do
  ...
  let(:order_type_map) do
    load_sul_orders_task.send(:order_type_mapping, 'acquisitions/order_type_map.tsv', Uuids.material_types)
  end
  let(:hldg_code_map) do
    load_sul_orders_task.send(:hldg_code_map, 'acquisitions/sym_hldg_code_location_map.tsv', Uuids.sul_locations)
  end
  let(:uuid_hashes) do
